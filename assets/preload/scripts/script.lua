...
json = {'bf', 'playable-sonic-exe', 'playable-sonic'} --json names
name = {'BoyFriend', 'Sonic.exe', 'Sonic'} --display names
...
function onStartCountdown() --enter selector mode
	...
	if not allowCountdown and not seenCutscene then
		...
		addCharacterToList('bf-dsides', 'boyfriend');
		precacheImage('characters/playables/BF D SIDES');
		addCharacterToList('playable-sonic-exe', 'boyfriend');
		precacheImage('characters/playables/TSE');
		addCharacterToList('playable-sonic', 'boyfriend');
		precacheImage('characters/playables/Sonic');
		addCharacterToList('gf-lullaby', 'boyfriend');
		precacheImage('characters/playables/Hypno Girlfriend');
		--end here
...
function onUpdatePost(elapsed)
	if inCharSelect == true then
		if variation == 1 then --change variation
			json = {'bf', 'playable-sonic-exe', 'playable-sonic'}
			name = {'BoyFriend', 'Sonic.exe', 'Sonic'}
		end
		if variation == 2 then --change variation
			json = {'bf-dsides', 'playable-sonic-exe', 'gf-lullaby'}
			name = {'Boyfriend D Sides (W.I)', 'Sonic.exe', 'Lullaby GF'}
		end
